...
router.get('/', auth, async (req, res) => {
    try {
        const roles = await Role.findAll({ where: { companyId: req.user.companyId } });
...
router.post('/', [auth, checkPermission('editRoles')], async (req, res) => {
    ...
    try {
        ...
        let role = await Role.findOne({ where: { name, companyId: req.user.companyId } });
        if (role) {
...
router.put('/:id', [auth, checkPermission('editRoles')], async (req, res) => {
    ...
    try {
        ...
        let role = await Role.findOne({ where: { id: req.params.id, companyId: req.user.companyId } });
