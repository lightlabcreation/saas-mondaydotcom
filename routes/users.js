...
router.put('/profile', auth, async (req, res) => {
  try {
    ...
    const user = await User.findOne({ where: { id: req.user.id, companyId: req.user.companyId } });
    if (!user) {
...
router.post('/', [auth, checkPermission('manageMembers')], async (req, res) => {
  try {
    ...
    let user = await User.findOne({ where: { email, companyId: req.user.companyId } });
    ...
    const userListData = {
      name, email, password: hashedPassword, role, avatar, phone, address, roleId, companyId: req.user.companyId
    };
    if (permissions) {
...
router.delete('/:id', [auth, checkPermission('manageMembers')], async (req, res) => {
  try {
    // if (req.user.role !== 'Admin') return res.status(403).json({ msg: 'Access denied' });
    await User.destroy({ where: { id: req.params.id, companyId: req.user.companyId } });
