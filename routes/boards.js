...
router.delete('/:id', auth, async (req, res) => {
  ...
  try {
    const board = await Board.findOne({ where: { id: boardId, companyId: req.user.companyId } });
...
router.get('/archived', auth, async (req, res) => {
  try {
    ...
    const boards = await Board.findAll({
      where: { isArchived: true, companyId: req.user.companyId },
      include: [{
        model: Group,
        as: 'Groups',
        required: false,
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // 1. Get all boards for this company
...
router.post('/', [auth, checkPermission('createMainBoards')], async (req, res) => {
  try {
    ...
    const boardData = { ...req.body, companyId: req.user.companyId };
    if (!boardData.ownerId) {
      boardData.ownerId = String(req.user.id);
    }

    const board = await Board.create(boardData);
    await Group.create({ title: 'New Group', BoardId: board.id, companyId: req.user.companyId });
    res.json(board);
  } catch (err) {
...
router.post('/:id/groups', [auth, checkBoardAccess], async (req, res) => {
  try {
    const group = await Group.create({ ...req.body, BoardId: req.params.id, companyId: req.user.companyId });
    res.json(group);
  } catch (err) {
    res.status(500).send('Server error');
  }
...
router.patch('/:id', [auth, checkBoardAccess], async (req, res) => {
  try {
    const board = await Board.findOne({ where: { id: req.params.id, companyId: req.user.companyId } });
