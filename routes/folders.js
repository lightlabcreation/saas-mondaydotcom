...
router.get('/', auth, async (req, res) => {
    try {
        ...
        const folders = await Folder.findAll({ where: { companyId: req.user.companyId } });
...
router.post('/', auth, async (req, res) => {
    try {
        ...
        const folder = await Folder.create({ name, companyId: req.user.companyId });
        res.json(folder);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.put('/:id', auth, async (req, res) => {
    try {
        const { name } = req.body;
        const folder = await Folder.findOne({ where: { id: req.params.id, companyId: req.user.companyId } });
        ...
        await Board.update({ folder: name }, { where: { folder: oldName, companyId: req.user.companyId } });
...
router.delete('/:id', auth, async (req, res) => {
    try {
        ...
        const { Board } = require('../models');
        const boardsInFolder = await Board.findAll({ where: { folder: folderName, companyId: req.user.companyId } });
