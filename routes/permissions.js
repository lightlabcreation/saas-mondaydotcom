...
router.post('/', auth, async (req, res) => {
    ...
    try {
        ...
        let permission = await Permission.findOne({ where: { key, companyId: req.user.companyId } });
        if (permission) return res.status(400).json({ msg: 'Permission key already exists' });

        permission = await Permission.create({ key, label, category, companyId: req.user.companyId });
        res.json(permission);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.put('/:id', auth, async (req, res) => {
    ...
    try {
        ...
        let permission = await Permission.findOne({ where: { id: req.params.id, companyId: req.user.companyId } });
