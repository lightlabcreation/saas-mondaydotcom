...
router.post('/start', auth, async (req, res) => {
    try {
        ...
        const activeSession = await TimeSession.findOne({
            where: { userId, itemId, isActive: true, companyId: req.user.companyId }
        });
...
router.post('/stop', auth, async (req, res) => {
    try {
        ...
        const allSessions = await TimeSession.findAll({
            where: { itemId, companyId: req.user.companyId }
        });
        ...
        if (!isNaN(itemId)) {
            await Item.update({ timeTracking: timeTrackingStr }, { where: { id: itemId, companyId: req.user.companyId } });
        }
...
router.get('/board/:boardId', auth, async (req, res) => {
    try {
        const board = await Board.findOne({
            where: { id: req.params.boardId, companyId: req.user.companyId },
            include: [{
                model: Group,
                as: 'Groups',
                include: [{
...
router.get('/active', auth, async (req, res) => {
    try {
        const sessions = await TimeSession.findAll({
            where: { userId: req.user.id, isActive: true, companyId: req.user.companyId },
