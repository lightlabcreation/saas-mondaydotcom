...
router.get('/board/:boardId', auth, async (req, res) => {
  try {
    ...
    const form = await Form.findOne({ where: { BoardId: targetBoardId, companyId: req.user.companyId } });
    if (!form) {
...
router.get('/', auth, async (req, res) => {
  try {
    const forms = await Form.findAll({ where: { companyId: req.user.companyId }, include: [{ model: Board, attributes: ['name'] }] });
    res.json(forms);
  } catch (err) {
    res.status(500).send('Server error');
  }
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    let form = await Form.findOne({ where: { BoardId: targetBoardId, companyId: req.user.companyId } });
    ...
    if (form) {
      ...
      form = await Form.create({ BoardId: targetBoardId, title, description, fields, settings, companyId: req.user.companyId });
    }
