...
const Subscription = sequelize.define('Subscription', {
  planType: { type: DataTypes.ENUM('Starter', 'Pro', 'Enterprise'), defaultValue: 'Starter' },
  status: { type: DataTypes.ENUM('active', 'trial', 'expired'), defaultValue: 'trial' },
...
const CompanyBackup = sequelize.define('CompanyBackup', {
  id: { type: DataTypes.BIGINT, primaryKey: true, autoIncrement: true },
  backupName: { type: DataTypes.STRING },
  backupSize: { type: DataTypes.STRING },
  status: { type: DataTypes.STRING, defaultValue: 'completed' },
  backupData: { type: DataTypes.JSON }, // Stores the massive JSON dump
}, {
  tableName: 'company_backups'
});
...
const SupportMessage = sequelize.define('SupportMessage', {
  ...
  senderId: { type: DataTypes.BIGINT, allowNull: false }, // User ID of sender
  receiverId: { type: DataTypes.BIGINT, allowNull: true }, // Superadmin ID, or null if broadcast
  isRead: { type: DataTypes.BOOLEAN, defaultValue: false }
}, {
...
// SaaS Isolations
Company.hasMany(CompanyBackup, { foreignKey: 'companyId', onDelete: 'CASCADE' });
CompanyBackup.belongsTo(Company, { foreignKey: 'companyId' });

User.hasMany(CompanyBackup, { foreignKey: 'createdBy', onDelete: 'SET NULL' });
CompanyBackup.belongsTo(User, { foreignKey: 'createdBy' });

Company.hasOne(Subscription, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Subscription.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(User, { foreignKey: 'companyId', onDelete: 'CASCADE' });
User.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Board, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Board.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Folder, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Folder.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Group, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Group.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Item, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Item.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Notification, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Notification.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(File, { foreignKey: 'companyId', onDelete: 'CASCADE' });
File.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Form, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Form.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Automation, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Automation.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Role, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Role.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Permission, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Permission.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(TimeSession, { foreignKey: 'companyId', onDelete: 'CASCADE' });
TimeSession.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(Payroll, { foreignKey: 'companyId', onDelete: 'CASCADE' });
Payroll.belongsTo(Company, { foreignKey: 'companyId' });

Company.hasMany(SupportMessage, { foreignKey: 'companyId', onDelete: 'CASCADE' });
SupportMessage.belongsTo(Company, { foreignKey: 'companyId' });
...
Item.hasMany(TimeSession, { as: 'timeSessions', foreignKey: 'itemId', constraints: false });
TimeSession.belongsTo(Item, { foreignKey: 'itemId', constraints: false });

// Associate session with the REAL parent record (for virtual subitems)
Item.hasMany(TimeSession, { as: 'childTimeSessions', foreignKey: 'parentItemId', constraints: false });
TimeSession.belongsTo(Item, { as: 'parentItem', foreignKey: 'parentItemId', constraints: false });
